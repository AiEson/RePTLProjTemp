...
class ResNestBottleneck(nn.Module):
    ...
    def __init__(
        ...
        if attention in {"2", "+", "M", "&"}:
            ...
            self.conv_for_DR_spatial = nn.Conv2d(
                planes * self.expansion, self.sp_d, kernel_size=1, stride=1, bias=True
            )
        ...
        if attention == "&":  # we employ a weighted spatial concat to keep dim
            ...
            self.dilate_conv_for_concat1 = nn.Conv2d(
                planes * self.expansion,
                planes * self.expansion,
                kernel_size=(self.conv_kernel_size, 1),
                stride=1,
                padding=(self.padding_num, 0),
                groups=self.groups,
                bias=True,
            )
            self.dilate_conv_for_concat2 = nn.Conv2d(
                planes * self.expansion,
                planes * self.expansion,
                kernel_size=(self.conv_kernel_size, 1),
                stride=1,
    ...
    def pos_att(self, out):
        ...
        out = out.view(
            out.size(0), out.size(1), out.size(2), 1
        ).contiguous()  # Nx64x64x1
...
softbce_fn = smp.losses.SoftBCEWithLogitsLoss(smooth_factor=0.1)
iou_fn = smp.losses.JaccardLoss(mode=smp.losses.BINARY_MODE, smooth=0.1)
...
def loss_fn(y_pred, y_true):
    ...
    # return bce
    # return bce
    return softbce_fn(y_pred, y_true) * 0.8 + dice_fn(y_pred.sigmoid(), y_true) * 0.2
...
def mask_onehot(masks):
    # batch_size, h, w
    masks = masks.long()
    masks_onehot = torch.zeros(
        masks.size(0), masks.max() + 1, masks.size(1), masks.size(2)
    ).to(masks.device)
...
class ResNeStGSoPUPnetPPModel(pl.LightningModule):
    ...
    def configure_optimizers(self):
        if self.args.optimizer == "adam":
            ...
            optimizer = torch.optim.Adadelta(
                self.parameters(), lr=self.args.learning_rate, weight_decay=1e-5
            )
        elif self.args.optimizer == "sgd":
            optimizer = torch.optim.SGD(
                self.parameters(), lr=self.args.learning_rate, weight_decay=1e-5
            )
        elif self.args.optimizer == "nmsgd":
            optimizer = torch.optim.SGD(
                self.parameters(),
                lr=self.args.learning_rate,
                weight_decay=1e-5,
        ...
        if self.args.scheduler == "reducelr":
            ...
            scheduler = lr_scheduler.CosineAnnealingWarmRestarts(
                optimizer, T_0=10, T_mult=1, last_epoch=-1, verbose=True
            )
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        f1 = f1_score(
            pre_label, mask, multiclass=True, num_classes=2, mdmc_average="global"
        )
        acc = accuracy(
            pre_label, mask, multiclass=True, num_classes=2, mdmc_average="global"
        )
        ...
        self.log(
            "train/f1", f1, on_epoch=True, on_step=True, prog_bar=False, sync_dist=True
        )
        self.log(
            "train/acc",
            acc,
    ...
    def validation_step(self, val_batch, batch_idx):
        ...
        self.log(
            "val/f1", f1, on_epoch=True, on_step=True, prog_bar=False, sync_dist=True
        )
        self.log(
            "val/acc", acc, on_epoch=True, on_step=True, prog_bar=False, sync_dist=True
        )
    ...
    def test_step(self, test_batch, batch_idx):
        ...
        self.log(
            "test/f1", f1, on_epoch=True, on_step=True, prog_bar=True, sync_dist=True
        )
        self.log(
            "test/acc", acc, on_epoch=True, on_step=True, prog_bar=True, sync_dist=True
        )
...
if __name__ == "__main__":
    model = ResNeStGSoPUPnetPPModel(
        encoder="resnest26d-meangsop-sca",
